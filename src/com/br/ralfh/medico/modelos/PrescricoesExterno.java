 ...
 * @author Ralfh
 */
public class PrescricoesExterno {
    
    public PrescricoesExterno() {
    }

    public static Boolean excluiPrescricao(PrescricaoExterno prescr) {
        Boolean resultado = Boolean.FALSE;
        try {
            EntityManager manager = JPAUtil.getEntityManager();
            manager.getTransaction().begin();  
            manager.remove(manager.getReference(PrescricaoExterno.class, prescr.getId()));  
    ...
    public static Boolean atualizaPrescricao(PrescricaoExterno prescr) {
        Boolean resultado = Boolean.FALSE;
        try {
            EntityManager manager = JPAUtil.getEntityManager();
            manager.getTransaction().begin();  
            manager.merge(manager.getReference(PrescricaoExterno.class, prescr.getId()));  
            manager.getTransaction().commit();
            manager.close();
            resultado = Boolean.TRUE;
        } catch (Exception ex) {
